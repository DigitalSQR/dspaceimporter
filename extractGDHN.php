...
foreach ($data as $nid => $item) {
	...
	if (!$main_title && count($titles) == 0) {	
		echo "\tWARNING: no titles found for $nid <$main_title>\n";  
		continue;
	}

	if (count($mimes) == 0) {	
		echo "\tWARNING: no mime types found for $nid\n"; 
		continue; 
	}
	...
	if (
		(count($item['lang']) > 0) && 
		($t_lang = $item['lang'][0]) && 
		(array_key_exists($t_lang,$lang_map))
		)
	{
   ...
   /* foreach ($aux_files as $filename => $mime) {
	...
	echo "\n\nWe here now\n\n";
	$file_list = array_unique(array_keys($titles));
	print_r($file_list);
	$zip->addFromString($dir . '/contents',implode($file_list,"\n") . "\n");
	
    foreach ($file_list as $f_name ) {
		echo "\tAdding $f_name\n";
		if (!file_exists(FURL . $f_name)) { 
			echo "WARNING: $f_name disappeared\n";
		}
		if (! $zip->addFile(FURL . $f_name, $dir . '/' . $f_name )) {
